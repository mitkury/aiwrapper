...
<div class="mx-auto w-full max-w-3xl">
	...
	>
		<textarea
			class="flex-1 resize-none border-0 bg-transparent text-neutral-900 placeholder:text-neutral-500 focus:ring-0 focus:outline-none"
			placeholder={providerReady
				? `Message ${providerName}...`
				: `Add ${providerName} settings to start chatting...`}
			bind:value={message}
		...
		{:else}
			<button
				...
				disabled={waitForResponse || message.length === 0 || !providerReady}
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					viewBox="0 0 24 24"
